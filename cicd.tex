Alps has five node node types described in \tbl{tab:alps-nodes}, on top of which CSCS partitions the system into vClusters\footnote{Versatile software defined cluster}~\cite{vClusters2023}, where each vCluster is a separate SLURM cluster that is customised for a tenant.
...
Uenv have minimal dependencies on the system, specifically:
\begin{itemize}
    \item libfabric: 
...
The information about versions, micro-architectures and the target system is captured in a ``label'' of the form \lst{name/version:tag@system\%uarch}
